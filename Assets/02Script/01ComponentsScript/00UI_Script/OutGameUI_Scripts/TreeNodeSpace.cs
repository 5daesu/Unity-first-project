...
public class TreeNodeSpace : MonoBehaviour  //TreeNodeGenerator also make sapce for treeNode
{
    ...
    private void GenerateTreeNode()
    {
        ...
        for (int i = 0; i < 4; i++)    //i is Unit's Level
        {
            ...
            foreach (UnitData unitData in unitDataList[i].unitDataList)     //index is Unit's order in same level
            {
                ...
                FillTreeNodeData(treeNode.GetComponent<TreeNodeObject>(), unitData);
    ...
    private void FillTreeNodeData(TreeNodeObject treeNodeObject, UnitData unitData)
    {
        treeNodeObject.unitData = unitData;
        treeNodeObject.unitCode = unitData.unitCode;
        treeNodeObject.unitImage.sprite = unitData.unitPrefab.GetComponent<SpriteRenderer>().sprite;
    }
    ...
    private GameObject FindTreeNode(int unitCode)
    {
        ...
        for (int i = 0; i < lvLength[unitLevel - 1]; i++)
        {
            ...
            if (unitCode == treeNodeArray[unitLevel - 1, i].GetComponent<TreeNodeObject>().unitCode) return treeNodeArray[unitLevel - 1, i];
        }
    ...
    {
        UnitData unitData = FindTreeNode(unitCode).GetComponent<TreeNodeObject>().unitData;
        ...
        else
        {
            ...
            {
                FindTreeNode(uCode).GetComponent<TreeNodeObject>().MarkTreeNode();
    ...
    private void FindParentTreeNode(int unitCode)
    {
        ...
        else
        {
            ...
            {
                uData = treeNodeArray[level, i].GetComponent<TreeNodeObject>().unitData;

                foreach (int uCode in DissolveMergeCode(uData.mergeCode, uData.mergeType))
                {
                    if (unitCode == uCode) treeNodeArray[level, i].GetComponent<TreeNodeObject>().MarkTreeNode();
                }
            }
        }
    }
